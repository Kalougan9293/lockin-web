...
import { isAmountColumnLabel, parseAmountToStorage } from "@/lib/preferences/currency-format";
import { isDateColumnLabel, parseDateInputToIso } from "@/lib/preferences/date-format";
import {
...
export function AddClientModal({ open, onClose, onSubmit }: AddClientModalProps) {
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    const filled = activeFields.some((label) => values[label]?.trim());

    if (!filled) {
      setError("Activez au moins un champ et renseignez une valeur.");
      return;
    }
